...
def _get_active_ex_or_ii(_cur_active, H, W, returning_active_ex=True):
    downsample_ratio_h = H // _cur_active.shape[2]
    downsample_ratio_w = W // _cur_active.shape[3]
    active_ex = _cur_active.repeat_interleave(downsample_ratio_h, 2).repeat_interleave(
        downsample_ratio_w, 3
    )
    return (
        active_ex
...
def sp_conv_forward(self, x):
    x, _cur_active = x
    x = super(type(self), self).forward(x)
    x *= _get_active_ex_or_ii(_cur_active,
        H=x.shape[2], W=x.shape[3],  returning_active_ex=True
    )  # (BCHW) *= (B1HW), mask the output of conv
    return [x, _cur_active]


def sp_bn_forward(self, x):
    x, _cur_active = x
    ii = _get_active_ex_or_ii(_cur_active, H=x.shape[2], W = x.shape[3], returning_active_ex=False)
    ...
    def forward(self, x):
        ...
        if x.ndim == 4:  # BHWC or BCHW
            if self.data_format == "channel_last":  # BHWC
                if self.sparse:
                    ii = _get_active_ex_or_ii(_cur_active, H=x.shape[1], W = x.shape[2], returning_active_ex=False)
                    nc = x[ii]
                ...
                if self.sparse:
                    ...
                    x[ii] = nc.to(x.dtype)
                    return [x.permute(0, 3, 1, 2), _cur_active]
                else:
    ...
    def forward(self, x):
        def _inner_forward(x):
            if self.sparse:
                ...
                x = self.norm(x)
                x[0] = self.pwconv1(x[0])
                x[0] = self.act(x[0])
                x[0] = self.pwconv2(x[0])
                x[0] = x[0].permute(0, 3, 1, 2)

                if self.gamma is not None:
                    x[0] = x[0].mul(self.gamma.view(1, -1, 1, 1))
                
                x[0] *= _get_active_ex_or_ii(x[1], H=x[0].shape[2], W = x[0].shape[3], returning_active_ex=True)

                x[0] = shortcut + self.drop_path(x[0])
                return x
...
def random_masking(B, H, W, ratio, device):
    ...
    idx = torch.rand(B, H * W, device=device).argsort(dim=1)
