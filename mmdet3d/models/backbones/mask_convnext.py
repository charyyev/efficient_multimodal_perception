...
class MaskConvNeXt(BaseModule):
    ...
    Args:
        ...
        mae_cfg (dict): Config for MAE
        downsample_height (bool): Whether to downsample in height dimension (range image does not downsamle height)
        multiscale (bool): Whether to use multiscale features
        out_all_scale (bool): Whether to ouput all features
    """  # noqa: E501
    ...
    def __init__(
        ...
        self.out_all_scale = out_all_scale
        if downsample_height:
            stem_kernel_size = stem_patch_size
            down_kernel_size = 2
        else:
            stem_kernel_size = stem_patch_size
            down_kernel_size = (1, 2)
        ...
        if multiscale:
            ...
            for i in range(len(self.channels)):
                self.reduce_dim.append(nn.Conv2d(self.channels[i], 768, kernel_size = 1))
    ...
    def create_masked_input(self, x):
        """Mask some part of input. 

        Args:
            x (torch.tensor): Input.
            
        Returns:
            list: Masked input and the mask.
        """

        if self.mae_cfg is None:
            return x, False
        
        B, _, H, W = x.shape
        # create masks on the smallest scale then upsample
        if self.downsample_height:
    ...
    def forward(self, x):
        ...
        if (self.mae_cfg is not None) and (not isinstance(x, list)):
            B, _, H, W = x.shape
            if self.downsample_height:
                downsample_scale_h = self.mae_cfg.downsample_scale * self.stem_patch_size[0]  # 32
                downsample_scale_w = self.mae_cfg.downsample_scale * self.stem_patch_size[1]
        ...
        if self.mae_cfg is None:
            B, _, H, W = inp.shape
            if self.downsample_height:
                downsample_scale_h = 32 
                downsample_scale_w = 32
            else:
                downsample_scale_h = 1
                downsample_scale_w = 32
            h, w = H // downsample_scale_h, W // downsample_scale_w
            return x, torch.zeros((B, 1, h, w), dtype = torch.bool, device = x.device)
