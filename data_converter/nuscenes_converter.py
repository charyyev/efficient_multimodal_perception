...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        lidar_filename = nusc.get('sample_data', sample['data']['LIDAR_TOP'])['filename']
        elev_filename = os.path.join("elevation", os.path.split(lidar_filename)[-1].split('.')[0]) + ".npz"
        
        range_filename = lidar_filename.replace('LIDAR_TOP', 'RANGE_FULL')
        range_filename = range_filename.replace('.pcd.bin', '') + ".npz"
